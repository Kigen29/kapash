...
export default function PitchDetailsScreen({ route, navigation }: any) {
  ...
  const { data: slots, isLoading: slotsLoading, refetch: refetchSlots } = usePitchSlots(pitchId, selectedDate);
  ...
  const handleBook = useCallback(() => {
    ...
    const slot = (slots as any[])?.find((s: any) => s.id === selectedSlotId);
    navigation.navigate('Checkout', {
      ...
      price: slot?.priceOverride || pitch?.pricePerHour,
      pitchImage: pitch?.images?.[0]?.url,
    });
  }, [selectedSlotId, slots, pitch, selectedDate]);
  ...
  if (error || !pitch) {
    return (
      <View style={s.loadingWrap}>
        <Text style={{ color: '#9CA3AF', fontSize: 16 }}>{error || 'Pitch not found'}</Text>
  ...
  const images = pitch.images?.length ? pitch.images.map((i: any) => i.url) : [
    ...
    <View style={s.container}>
      ...
      <View style={s.imgWrap}>
        <FlatList
          ...
          onScroll={e => setImgIndex(Math.round(e.nativeEvent.contentOffset.x / e.nativeEvent.layoutMeasurement.width))}
          renderItem={({ item }) => <Image source={{ uri: item }} style={s.heroImg} />}
        />
        <LinearGradient colors={['rgba(0,0,0,0.5)', 'transparent']} style={s.imgTopGrad} />
        <LinearGradient colors={['transparent', 'rgba(15,25,35,0.95)']} style={s.imgBotGrad} />
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.badgeRow}>
          {pitch.rating && (
            <View style={s.badge}>
              <Text style={s.badgeTxt}>⭐ {pitch.rating.toFixed(1)} ({pitch._count?.reviews || 0})</Text>
            </View>
          )}
          <View style={s.badge}>
          ...
          {pitch.instantBook && (
            <View style={[s.badge, s.greenBadge]}>
              <Text style={[s.badgeTxt, { color: '#22C55E' }]}>⚡ Instant Book</Text>
            </View>
          )}
        </View>
            ...
            <View style={s.amenitiesWrap}>
              {amenities.map((a: any) => (
                <View key={a.id} style={s.amenityChip}>
                  <Text style={s.amenityTxt}>{a.icon || '✓'} {a.name}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View style={s.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {DATES.map(d => (
              ...
              >
                <Text style={[s.dateLbl, selectedDate === d.value && s.dateLblActive]}>{d.label}</Text>
                <Text style={[s.dateDay, selectedDate === d.value && s.dateDayActive]}>{d.day}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
            ...
            <View style={s.slotsGrid}>
              {slotList.map((slot: any) => {
                ...
                return (
                  <TouchableOpacity
                    key={slot.id}
                    style={[s.slot, booked && s.slotBooked, selected && s.slotSelected]}
                    onPress={() => !booked && setSelectedSlotId(slot.id)}
                    disabled={booked}
                  >
                    <Text style={[s.slotTime, booked && s.slotTimeBooked, selected && s.slotTimeSelected]}>
                      {formatTime(slot.startTime)} – {formatTime(slot.endTime)}
                    </Text>
                    {slot.priceOverride && (
                      <Text style={[s.slotPrice, selected && { color: '#fff' }]}>
                        KSh {slot.priceOverride?.toLocaleString()}
                      </Text>
                    )}
      ...
      <View style={s.footer}>
        <View>
          ...
          <Text style={s.footerPrice}>KSh {pitch.pricePerHour?.toLocaleString()}</Text>
        </View>
        <TouchableOpacity onPress={handleBook} style={s.bookBtnWrap} activeOpacity={0.85}>
          <LinearGradient colors={['#22C55E', '#16A34A']} style={s.bookBtn} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const s = StyleSheet.create({
  ...
  loadingWrap: { flex: 1, backgroundColor: '#0F1923', justifyContent: 'center', alignItems: 'center' },
  ...
  backBtn: {
    margin: 16, width: 36, height: 36, backgroundColor: 'rgba(0,0,0,0.5)',
  ...
  pitchName: { fontSize: 20, fontWeight: '700', color: '#fff', marginBottom: 4 },
  pitchLoc: { fontSize: 13, color: '#9CA3AF' },
  price: { fontSize: 22, fontWeight: '800', color: '#22C55E' },
  perHr: { fontSize: 12, color: '#6B7280' },

  badgeRow: { flexDirection: 'row', gap: 8, paddingHorizontal: 16, marginBottom: 8, flexWrap: 'wrap' },
  badge: {
    backgroundColor: '#1A2535', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5,
  },
  ...
  amenityChip: { backgroundColor: '#1A2535', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
  amenityTxt: { color: '#D1D5DB', fontSize: 12, fontWeight: '600' },

  dateBtn: {
    width: 60, height: 70, backgroundColor: '#1A2535', borderRadius: 12,
    alignItems: 'center', justifyContent: 'center', marginRight: 8,
    borderWidth: 1, borderColor: '#374151',
  },
  dateBtnActive: { borderColor: '#22C55E', backgroundColor: 'rgba(34,197,94,0.1)' },
  dateLbl: { fontSize: 10, color: '#6B7280', fontWeight: '600', marginBottom: 4 },
  ...
  slot: {
    width: '47%', backgroundColor: '#1A2535', borderRadius: 10,
    paddingVertical: 12, paddingHorizontal: 10, borderWidth: 1, borderColor: '#374151',
  },
  ...
  noSlots: { color: '#6B7280', fontSize: 14, textAlign: 'center', paddingVertical: 24 },

  footer: {
    position: 'absolute', bottom: 0, left: 0, right: 0,
    backgroundColor: '#1A2535', borderTopWidth: 1, borderTopColor: '#374151',
  ...
  bookBtnWrap: { flex: 0 },
  bookBtn: { paddingHorizontal: 32, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
