...
import { useOwnerDashboard, useOwnerBookings } from '../../hooks/useData';
...
export default function OwnerDashboardScreen({ navigation }: any) {
  ...
  const { data: todayBookings, refetch: refetchBookings } = useOwnerBookings({
    date: new Date().toISOString().split('T')[0],
  });

  const onRefresh = async () => {
    setRefreshing(true);
    await Promise.all([refetchDash(), refetchBookings()]);
    setRefreshing(false);
  };

  const bookings: any[] = todayBookings || [];
  const d = dash || {};
    ...
    <View style={s.container}>
      ...
      >
        {isLoading ? (
          ...
          <>
            {/* Revenue Hero */}
            <LinearGradient colors={['#16A34A', '#22C55E']} style={s.heroCard} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
              <Text style={s.heroLabel}>This Month's Revenue</Text>
              <Text style={s.heroAmount}>KSh {(d.monthlyRevenue || 0).toLocaleString()}</Text>
            ...
            <View style={s.statsGrid}>
              <StatCard icon="📅" label="Today's Bookings" value={d.todayBookings || 0} color="#22C55E" />
              <StatCard icon="⏳" label="Pending" value={d.pendingBookings || 0} color="#F59E0B" />
              <StatCard icon="⭐" label="Avg Rating" value={`${(d.avgRating || 0).toFixed(1)}★`} color="#F59E0B" />
              <StatCard icon="💰" label="Pending Payout" value={`KSh ${(d.pendingPayout || 0).toLocaleString()}`} color="#22C55E" />
            </View>
            ...
            <View style={s.section}>
              ...
              <View style={s.actionsRow}>
                <ActionBtn icon="📆" label="Schedule" onPress={() => navigation.navigate('ManageSchedule')} />
                <ActionBtn icon="📊" label="Analytics" onPress={() => navigation.navigate('Analytics')} />
                <ActionBtn icon="🏟️" label="My Pitches" onPress={() => navigation.navigate('ManagePitches')} />
                <ActionBtn icon="💸" label="Payouts" onPress={() => navigation.navigate('OwnerAccount')} />
              </View>
            </View>
...
function ActionBtn({ icon, label, onPress }: { icon: string; label: string; onPress: () => void }) {
  return (
    <TouchableOpacity style={s.actionBtn} onPress={onPress} activeOpacity={0.8}>
      <View style={s.actionIconWrap}><Text style={s.actionIcon}>{icon}</Text></View>
  ...
  return (
    <View style={s.bookingRow}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={s.bookingSlot}>{booking.timeSlot?.startTime} – {booking.timeSlot?.endTime}</Text>
      </View>
      <Text style={[s.bookingStatus, { color }]}>{booking.status}</Text>
      <Text style={s.bookingAmt}>KSh {booking.totalAmount?.toLocaleString()}</Text>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  greeting: { fontSize: 12, color: '#9CA3AF', fontWeight: '600', textTransform: 'uppercase', letterSpacing: 1 },
  subGreet: { fontSize: 20, fontWeight: '700', color: '#fff' },
  notifBtn: { width: 40, height: 40, backgroundColor: '#1A2535', borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  ...
  heroAmount: { fontSize: 32, fontWeight: '800', color: '#fff', marginBottom: 16 },
  heroRow: { flexDirection: 'row', justifyContent: 'space-between' },
  miniStat: { alignItems: 'center' },
  miniStatVal: { fontSize: 15, fontWeight: '700', color: '#fff' },
  miniStatLabel: { fontSize: 10, color: 'rgba(255,255,255,0.7)', marginTop: 2 },

  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginHorizontal: 16, marginBottom: 20 },
  statCard: {
    width: '47%', backgroundColor: '#1A2535', borderRadius: 14,
  ...
  actionIconWrap: {
    width: 52, height: 52, backgroundColor: '#1A2535', borderRadius: 14,
  ...
  emptyBox: { backgroundColor: '#1A2535', borderRadius: 12, padding: 24, alignItems: 'center' },
