...
export async function apiFetch<T = any>(
  ...
  const data: T = ct?.includes('application/json') ? await res.json() : await res.text() as any;
...
export const PITCHES = {
  ...
  getSlots: (pitchId: string, date: string) =>
    apiFetch(`/pitches/${pitchId}/slots?date=${date}`),
...
export const OWNER = {
  ...
  updateSlot: (slotId: string, body: { status: string }) =>
    apiFetch(`/owner/slots/${slotId}`, { method: 'PATCH', body: JSON.stringify(body) }),
  requestPayout: (amount: number) =>
    apiFetch('/owner/payouts/request', { method: 'POST', body: JSON.stringify({ amount }) }),
};
